...
interface Upgrade {
  id: string
  name: string
  category: "solar" | "hvac" | "roofing" | "windows" | "insulation"
...
function computeUpgrades(a: Answers): Upgrade[] {
  ...
  if (noSolar) {
    ...
    const estimatedCost = systemCost
    const paybackYrs = Math.round(estimatedCost / annualSavings)
    upgrades.push({
      id: "solar",
      name: "Solar Panel System",
      category: "solar",
      icon: <Sun className="w-6 h-6" />,
      cost: `$${estimatedCost.toLocaleString()} before incentives`,
      annualSavings: `$${annualSavings.toLocaleString()}`,
      payback: `${paybackYrs}-${paybackYrs + 2} years`,
      roi25yr: `$${(annualSavings * 25 - estimatedCost).toLocaleString()}`,
      why: `Based on your $${bill}/mo bill, solar may reduce a large share of your electricity costs. Current incentive eligibility depends on project timing, utility rules, state programs, and quote details.`,
      incentiveNote: "Verify state, utility, and current tax eligibility",
  ...
  if (hvacOld || hvacModerate) {
    ...
    upgrades.push({
      ...
      why: hvacOld
        ? `Your system is likely running at 60–70% efficiency. A modern heat pump runs at 300%+ efficiency, cutting heating/cooling costs by ~25% while utility or state rebates may apply depending on equipment and local program rules.`
      ...
      incentiveNote: "Verify current utility, state, and tax rules",
  ...
  if (singlePane || a.goals.includes("comfort")) {
    ...
    upgrades.push({
      ...
      why: singlePane
        ...
        : `Energy-efficient windows reduce hot/cold spots, cut UV damage to furniture, and may qualify for utility or state rebates depending on current program rules.`,
      incentiveNote: "Verify current utility, state, and tax rules",
...
const CATEGORY_STYLES = {
  ...
  insulation: { bg: "bg-amber-50", border: "border-amber-200", dot: "bg-amber-500", cta: "insulation" },
}
...
export default function UpgradePlannerClient() {
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500 max-w-xl mx-auto">
            Based on your home profile, here are quote-ready upgrade options ranked by likely impact and urgency.
          </p>
        </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {u.incentiveNote && (
                      <div className="flex items-center gap-2 text-xs text-green-700 font-semibold mb-4">
                        ...
                        Incentive check: {u.incentiveNote}
                      </div>
                    )}
        ...
        <div className="bg-gray-900 text-white rounded-2xl p-8 text-center mb-8">
          ...
          <p className="text-gray-500 text-xs mt-3">
            Enter your ZIP to compare local quote options and current rebate assumptions.
          </p>
        </div>
    ...
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 bg-green-100 text-green-800 text-sm font-semibold px-4 py-1.5 rounded-full mb-3">
          <Zap className="w-4 h-4" /> Free Upgrade Planner
        </div>
        <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-2">
          Your Home Upgrade Planner
        </h1>
        <p className="text-gray-500">
          Answer 4 quick questions. Get a quote-ready upgrade roadmap ranked by likely impact, urgency, and current incentive checks.
        </p>
      </div>
